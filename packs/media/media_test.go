...
import (
	...
	"github.com/subosito/mow/packs/media"
)
...
func TestUnderstandImageTool(t *testing.T) {
	...
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{Client: mc, Workspace: ws, UnderstandImage: "vl-m"})
...
func TestImageGenToolWritesMediaDir(t *testing.T) {
	...
	ws := t.TempDir()
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{Client: mc, Workspace: ws, GenerateImage: "img-m"})
...
func TestUnderstandVoiceTool(t *testing.T) {
	...
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{Client: mc, Workspace: ws, UnderstandVoice: "whisper"})
...
func TestSpeechGenToolWritesMediaDir(t *testing.T) {
	...
	ws := t.TempDir()
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{Client: mc, Workspace: ws, GenerateSpeech: "tts-m"})
...
func TestSpeechGenToolUsesConfiguredDefaultVoice(t *testing.T) {
	...
	ws := t.TempDir()
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{
		Client: mc, Workspace: ws, GenerateSpeech: "tts-m", DefaultSpeechVoice: "VoiceX",
	})
...
func TestVideoGenToolBytesPath(t *testing.T) {
	...
	ws := t.TempDir()
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{Client: mc, Workspace: ws, GenerateVideo: "vid-m"})
...
func TestUnderstandVideoTool(t *testing.T) {
	...
	mc := &mow.MediaClient{BaseURL: srv.URL + "/v1", APIKey: "k"}
	list := media.MediaTools(media.MediaOptions{Client: mc, Workspace: ws, UnderstandVideo: "v-m"})
...
func TestMediaToolsAllConfigured(t *testing.T) {
	list := media.MediaTools(media.MediaOptions{
		Client:          &mow.MediaClient{BaseURL: "http://x", APIKey: "k"},
		Workspace:       t.TempDir(),
		GenerateImage:   "a",
		GenerateSpeech:  "b",
		GenerateVideo:   "c",
