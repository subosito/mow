...
func (c *Client) chatAnthropic(ctx context.Context, messages []Message, tools []ToolSpec) (Message, error) {
	...
	msg.Usage = Usage{
		...
		// Reads and writes are tracked separately: a read is a discount, a
		// write is a surcharge. Folding them together would hide the write
		// amplification that dominates a long session's bill.
		CachedInputTokens:     parsed.Usage.CacheReadInputTokens,
		CacheWriteInputTokens: parsed.Usage.CacheCreationInputTokens,
	}
