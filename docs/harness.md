...
| Knob | Default | Effect |
|------|---------|--------|
| `policy.max_turns` | `120` | Runaway guard: LLM round-trips per Prompt (a turn may batch up to 8 tools). `0` or yaml `-1` = unlimited. CLI: `--max-turns 0`. Exhausted runs stop with `max_turns`; the session resumes with a follow-up prompt |
